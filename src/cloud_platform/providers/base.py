...
class OrderRecoveryVerdict(StrEnum):
    ...
    ``MATCHED``: the adapter PROVED that exactly one order belongs to the
    local operation via a provider-side identity (e.g. a client reference /
    true idempotency key). Generic price/time similarity is NEVER proof of
    ownership: an adapter whose API cannot expose such an identity must
    return ``AMBIGUOUS`` instead of ``MATCHED``. ``NO_MATCH``: a clean
    scan found no candidate (absence can NOT be proven: the order may be
    invisible yet, or the scan window is limited — the platform escalates
    to human review). ``AMBIGUOUS``: one or more candidates match the
    provider-side facts but none can be proven to belong to this
    operation — a human must decide; the platform never guesses.
    ``SCAN_FAILED``: the scan itself could not complete (transient); the
    platform retries it a bounded number of times before escalating.
    """
...
class OrderingProvider(Protocol):
    ...
    async def recover_order(
        ...
        *,
        ...
        of provider correlation. A candidate may be reported MATCHED only
        when the provider API exposes an identity that PROVES the order
        belongs to this operation (client reference / idempotency identity);
        generic similarity (price/term/cycle/time) is NEVER sufficient —
        with only generic facts, one candidate is AMBIGUOUS just like
        several, and zero is NO_MATCH (absence not provable).
        """
        ...
